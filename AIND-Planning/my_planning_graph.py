...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            #Create Planning Graph node for this action
            node_a = PgNode_a(action)
            # This action node has prenodes that are a subset of the previous S level
            if node_a.prenodes.issubset(self.s_levels[level]):
                ...
                # Added action node to current level in self.a_levels[level]
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set())  # S0 set of s_nodes - empty to start
        # Get a node in the previous level has a list
        for node_a in self.a_levels[level -1]:
            # Get list of S nodes from effnodes that represent by the action node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (set(node_a1.action.effect_add) and set(node_a2.action.effect_rem)) or \
               (set(node_a2.action.effect_add) and set(node_a1.action.effect_rem))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (set(node_a1.action.effect_add) and set(node_a2.action.precond_neg))  or \
               (set(node_a1.action.effect_rem) and set(node_a2.action.precond_pos))  or \
               (set(node_a2.action.effect_add) and set(node_a1.action.precond_neg))  or \
