    ...
    def select(self):
        ...
        try:
            for num_states in num_of_components:
             ...
             pass
        num_states = num_of_components[np.argmin(bic_scores)] if bic_scores else self.n_constant
    ...
    def select(self):
        ...
        try:
            for num_states in num_of_components:
                ...
                antiLogL /= float(wc)
                logs_likelihood[num_states] = log_likelihood
                antiRes[num_states] = antiLogL

            for i in num_of_components:
                dic_score = logs_likelihood[i] - antiRes[i]
